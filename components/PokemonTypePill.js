...
const PokemonTypePill = ({ typeName = "", icon = true, text = true, selected = true, ...props }) => {
    let typeIcon = null;
    if (icon) {
        try {
    ...
    return (
        <button
            className={`inline-flex flex-row justify-items-center items-center font-bold py-1 px-2 rounded full capitalize ${
                styles["type-pill"]
            } ${!selected && styles["type-pill--plain"]} bg--${typeName.toLowerCase()} ${
                !selected && `bg--${typeName.toLowerCase()}--noFill`
            }`}
            type="button"
            {...props}
        >
